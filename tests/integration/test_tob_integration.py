...
class TestTOBIntegration:
    ...
    def test_open_tob_file_success(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             patch.object(controller.tob_service, 'load_tob_file', return_value=mock_tob_data), \
             patch.object(controller.data_service, 'process_tob_data', return_value={}), \
             patch.object(controller.data_service, '_calculate_metrics', return_value={}):
    ...
    def test_open_tob_file_validation_error(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=False):
    ...
    def test_open_tob_file_load_error(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             patch.object(controller.tob_service, 'load_tob_file', side_effect=FileNotFoundError("File not found")):
    ...
    def test_file_load_progress_signals(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             patch.object(controller.tob_service, 'load_tob_file') as mock_load, \
             patch.object(controller.data_service, 'process_tob_data') as mock_process:
    ...
    def test_data_processing_integration(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             patch.object(controller.tob_service, 'load_tob_file', return_value=mock_tob_data), \
             patch.object(controller.data_service, 'process_tob_data', return_value={"processed": True}), \
             patch.object(controller.data_service, '_calculate_metrics', return_value={"metric": 123}):
    ...
    def test_view_update_integration(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             ...
             patch.object(controller.data_service, 'process_tob_data', return_value={}), \
             patch.object(controller.data_service, '_calculate_metrics', return_value={}), \
             patch.object(controller.main_window, 'update_project_info') as mock_update_project, \
             patch.object(controller.main_window, 'update_data_metrics') as mock_update_metrics, \
             patch.object(controller.main_window.ui_state_manager, 'show_plot_mode') as mock_show_plot:
    ...
    def test_error_handling_integration(self, mock_controller):
        ...
        with patch.object(controller.tob_service, 'validate_tob_file', return_value=True), \
             patch.object(controller.tob_service, 'load_tob_file', side_effect=Exception("Test error")), \
             patch.object(controller.error_handler, 'handle_error') as mock_handle_error:
