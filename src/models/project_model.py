...
class ServerConfig(BaseModel):
    ...
    project_field_name: str = Field(default="project", description="Form field name for project")
    location_field_name: str = Field(default="location", description="Form field name for location")
    tob_file_field_name: str = Field(default="tob_file", description="Form field name for TOB file")
    subconn_length_field_name: str = Field(default="subcon", description="Form field name for subcon length")
    string_id_field_name: str = Field(default="string_id", description="Form field name for string ID")
    comment_field_name: str = Field(default="comment", description="Form field name for comment")
...
class TOBFileInfo(BaseModel):
    ...
    added_date: datetime = Field(default_factory=datetime.now, description="Date when file was added")
...
class ProjectModel(BaseModel):
    ...
    server_config: Optional[ServerConfig] = Field(None, description="Server configuration")
    tob_files: List[TOBFileInfo] = Field(default_factory=list, description="TOB files in project")
    project_data: Dict[str, Any] = Field(default_factory=dict, description="Processed project data")
    encryption_key: Optional[str] = Field(None, description="Encryption key for the project")
    
    def add_tob_file(self, file_path: str, file_name: str, file_size: int, 
                    data_points: Optional[int] = None, sensors: Optional[List[str]] = None) -> None:
        """
