...
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QDoubleSpinBox, QFrame, QSpacerItem,
...
class MainWindow(QMainWindow):
    ...
    project_created = pyqtSignal(str, str)  # Emitted when project is created (path, password)
    project_opened = pyqtSignal(str, str)  # Emitted when project is opened (path, password)
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            self.error_handler.show_error(
                "UI Loading Error",
                f"Failed to load the user interface: {e}"
            )
            raise
    ...
    def _store_widget_references(self):
        ...
        self.welcome_open_tob_button = self.findChild(QPushButton, "welcome_open_tob_button")
        self.welcome_open_project_button = self.findChild(QPushButton, "welcome_open_project_button")
        ...
        self.location_subcon_spin = self.findChild(QDoubleSpinBox, "location_subcon_spin")
        self.location_comment_value = self.findChild(QLineEdit, "location_comment_value")
        self.location_sensorstring_value = self.findChild(QLineEdit, "location_sensorstring_value")
        
        # Action buttons
        self.quality_control_button = self.findChild(QPushButton, "quality_control_button")
        self.send_data_button = self.findChild(QPushButton, "send_data_button")
        self.request_status_button = self.findChild(QPushButton, "request_status_button")
    ...
    def _initialize_axis_controls(self):
        ...
        sensor_options = ["NTC01", "NTC02", "NTC03", "NTC04", "NTC05", "NTC06", "NTC07",
                         "NTC08", "NTC09", "NTC10", "NTC11", "NTC12", "NTC13", "NTC14",
                         "NTC15", "NTC16", "NTC17", "NTC18", "NTC19", "NTC20", "NTC21",
                         "NTC22", "PT100"]
    ...
    def _setup_menu_bar(self):
        ...
        self.actionCreate_Project_File = self.findChild(QAction, "actionCreate_Project_File")
        self.actionOpen_Project_File = self.findChild(QAction, "actionOpen_Project_File")
        self.actionEdit_Project_Settings = self.findChild(QAction, "actionEdit_Project_Settings")
        self.actionShow_Processing_List = self.findChild(QAction, "actionShow_Processing_List")
    ...
    def _connect_signals(self):
        ...
        for sensor_name, checkbox in self.ntc_checkboxes.items():
            checkbox.stateChanged.connect(lambda state, name=sensor_name: self._on_sensor_selection_changed(name, state))
    ...
    def _on_open_tob_file(self):
        ...
        try:
            ...
            self.error_handler.show_error("File Open Error", f"Failed to open file: {e}")
    ...
    def _on_open_project(self):
        ...
        try:
            ...
            self.error_handler.show_error("Project Open Error", f"Failed to open project: {e}")
    ...
    def _on_create_project(self):
        ...
        try:
            ...
            self.error_handler.show_error("Project Creation Error", f"Failed to create project: {e}")
